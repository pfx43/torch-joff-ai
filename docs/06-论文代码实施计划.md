...
状态：**P1-P4 已完成；当前进入 P5 受保护参考、监视器状态和堆叠残差**
...
三方验证。当前实施入口是第 13 节 P5：

1. P5 只在评估层实现受保护参考和监视状态，不把告警状态放回 P4 模型；
2. 数据支路可读取真实记录，受保护支路的 post-anchor measurement slot 只能写预测；
3. 实现 anchor gate、hysteresis、reset、可序列化 trace 和 episode 边界；
4. 生成堆叠受保护残差，但不提前宣称 P6 算子认证或 P8 动态阈值已完成；
5. 通过 P5 相关验证和双轴审查后形成独立提交，再进入 P6。
