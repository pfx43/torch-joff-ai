...
source hash 与 DataModule sequence 边界修正。后续直接从 P2 开始：

1. 以第 10 节为规格选择 FiveStageNormalSplitter 的公开 API；
2. 从原始时间轴不重叠与隔离带行为的失败测试开始 TDD；
3. 完成访问账本、manifest、相关验证和双轴审查；
4. 创建 P2 独立提交后再进入 P3。
