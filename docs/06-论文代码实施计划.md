...
状态：**P1-P7 已完成实现、验证、双轴审查和独立提交；当前实施 P8 动态阈值**
...
1. 只在 normal estimate episode 拟合输入/输入变化/运行区域和 reference-age 包络；
2. 通过同一个 P7 ``L_b`` 传播 ``G_nu`` block columns，分别记录 ``gamma_anc`` 和
   ``gamma_det``；
3. 在 detection-calibration episode 上冻结 family-wise maximum，并按有限秩规则计算
   ``q_det``；
4. 请求风险低于 ``1/(n_cal+1)``、descriptor 不覆盖或证据缺失时返回正无穷并禁用决定；
5. detection quantile 不得反馈 anchor、mode、window 或 score path，且不得读取
   attribution calibration 或故障数据。
