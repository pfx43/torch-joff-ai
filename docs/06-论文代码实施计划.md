...
状态：**P1-P10 软件、CSTR 许可核验和正常-only development 已完成；formal 认证 runtime
仍缺失，P11 执行门保持关闭**
...
三条款 BSD 文本；公开数据卡已保存来源 URL、版本、许可证 URL、许可证文本 SHA-256 和本地
`cstr_close.slx` SHA-256，两份论文配置同步声明 `license_status=verified`。正常与故障 MAT
仍由配置中的各自 SHA-256 固定，且原始数据继续不纳入 Git。

该许可核验只解决“是否允许使用”的前置事实，不把 development 产物变成论文结果，也不替代
P6/P9 的真实认证 provider、formal runtime、一次性 claim 或完整产物复验。
...
真实 CSTR frozen evaluation 尚未执行。许可证已经核实，正常-only development 也已生成
20 个 P2--P9 产物并完成 checkpoint replay；但这些产物的 evaluator 明确保存
`formal_pipeline_complete=false`，operator/signature/nuisance 仍未获得真实
interval/verified-quadrature 与 full nonlinear provider 的认证。正式 CLI 因而在 manifest、
claim 和 fault value 访问前拒绝 development-only runtime，P11 不得以“P10 已产生正式结果”
为前提启动。
...
第 18 节 P10 的软件工作流已经完成最终审查、整改、全套验证和独立提交。CSTR 许可证据与
正常-only P2--P9 development 已于 2026-07-30 闭环；当前 preflight 只剩 restored evaluator
为 development-only 的正式 runtime 阻塞。仍须接入真实认证 provider，重新生成并复核
`formal_pipeline_complete=true` 的正常产物，再执行一次性 CSTR 正式评价。P11 只完成第
19.1 节的数据合同与执行前门禁准备，检查点配置仍处于阻塞态，未读取 TTS 真实故障值、未运行
