<!--
文件用途：把当前英文论文中的方法要求转换成可执行、可测试、可分阶段提交的代码实施路线。
主要职责：说明实施顺序、模块边界、候选公开接口、TDD 验收点、运行产物、停止条件和 Git 提交策略；
不负责宣称论文理论已经定稿，也不记录尚未运行的实验数值。
关键输入与输出：输入为 MANUSCRIPT_CONTEXT.md、paper/main.tex、Joff 现有架构和已确认的用户决策；
输出为后续 .scratch 规格/工单和逐阶段实现的上位计划。
依赖与副作用：本文只描述计划，不执行训练、不读取冻结故障性能、不生成论文结果。
重要约束：所有拟合、选择和校准只使用允许的正常数据；真实故障数据只在协议冻结后用于最终评价；
理论未闭合处必须显式返回未认证状态，不能用占位算法冒充证明。
-->

# 论文代码实施计划

状态：**阶段 0 规划已确认，方法代码尚未开始实现**
...
1. 从本计划生成 `.scratch/<阶段>/spec.md` 和多个小工单；
2. 与用户确认本阶段的公开测试入口；
...
7. 阶段功能完成后运行相关回归测试；
8. 以阶段开始前的提交为 fixed point，分别做规范审查和规格审查；
9. 修复审查问题并重新验证；
10. 在 `main` 创建本阶段提交；
11. 下一阶段只从这个已提交的稳定点开始。

TDD 中的“公开入口”必须在该阶段开始时再次确认。本计划给出的接口是候选设计，不等于已经实现。
...
- `NominalJVPAssembler`：提供可测试的名义算子；
- `CertifiedEnclosureProvider`：认证后端协议；
- `OperatorEnclosure`：中心、误差半径、共享不确定元素和来源。
...
用户明确发出“开始 P1”后，才进入真正实现。第一步不是立刻改适配器，而是：

1. 创建 `.scratch/cstr-protocol-repair/spec.md`；
2. 拆分 P1 工单；
3. 再次核对 onset 和变量顺序的权威来源；
4. 与用户确认 P1 的公开测试入口；
5. 从第一个失败测试开始 TDD；
6. 完成 P1 审查、验证和 Git 提交后，再讨论 P2。
