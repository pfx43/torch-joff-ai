...
状态：**P1-P5 已完成；当前进入 P6 堆叠算子装配与认证状态**
...
P5 已完成严格分离的 data/protected 双支路、延迟 anchor gate、hysteresis、重锚/stale、
受控正常/冻结故障阶段、联合 monitor 身份、可序列化状态/trace 和同边界同锚点堆叠残差。
固定记录命令时，post-anchor 真实测量无法进入受保护命令或 measurement buffer；自动接受
锚点始终保持 ``UNVERIFIED``，不把无报警冒充正常性证书。当前实施入口是第 14 节 P6：

1. 装配 ``OperatorBundle``，但把 exact identity、名义数值和认证 enclosure 明确分层；
2. 在线名义 JVP 只能产生 ``nominal``，没有 provider 时必须是 ``unavailable`` 而非伪造认证；
3. 线性二维系统恢复块下三角 ``G_nu``，并验证单位对角块、可逆性和 exact propagation；
4. 所有支路共享同一 operator-error 元素，禁止按支路独立选择最有利误差；
5. 通过 P6 相关验证和双轴审查后形成独立提交，再进入 P7。
