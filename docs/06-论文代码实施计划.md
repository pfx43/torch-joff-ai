...
状态：**P1-P3 已完成；当前进入 P4 严格过去的 Attention--Koopman--T--S 正常模型**
...
正常基线、静态 episode 校准、checkpoint 重放及可追溯产物；双轴审查发现的 episode
跨界、配置默认、追溯 hash、数据层所有权和中文契约问题均已整改，相关 50 项回归通过。
当前实施入口是第 12 节 P4：

1. 在模型外保持告警、在线状态机和实验目录职责，P4 只解决正常建模；
2. 实现严格过去 causal attention、可复现 channel mask、可学习 T--S 局部 Koopman；
3. 训练和在线复用同一自由多步 rollout，第二步起不得读取真实未来输出；
4. 用解析公式、autograd 和中心有限差分三方验证包含 membership variation 的完整 Jacobian；
5. 通过 P4 相关验证和双轴审查后形成独立提交，再进入 P5。
