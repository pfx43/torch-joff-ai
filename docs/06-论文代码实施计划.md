...
状态：**P1-P10 软件阶段已完成独立提交；正式冻结评价仍被外部前置条件阻断，P11 尚未启动**
...
第 18 节 P10 的软件工作流已经完成最终审查、整改、全套验证和独立提交。真实一次性故障评价仍须先核实 CSTR
许可，生成并复核完整正常产物，并接入真实认证 provider；当前 preflight 会在 manifest、
claim 和 fault value 访问前列出这些阻塞。不得用 synthetic smoke、development-only
checkpoint、线性特例或正常校准结果填充论文性能数值；在上述前置条件闭环前暂不进入 P11。
