...
状态：**P1、P2 已完成；当前实施 P3 论文协议与最小正常数据基线**
...
P1 已核对随数据说明中的 onset 和变量顺序，并完成物理 schema、逐行标签、episode 追溯、
source hash 与 DataModule sequence 边界修正。P2 已按第 10 节完成 FiveStageNormalSplitter、
隔离带、窗口依赖、完整 calibration episode、FitAccessLedger、manifest 和冻结故障门禁；
相关 74 项回归中 68 项通过、6 项跳过，P2 模块定向 mypy、Ruff、安静导入和差异检查通过，
最终双轴复审无未解决高优先级问题。当前实施入口是：

1. 以第 11 节为规格实现独立论文协议编排，不修改通用 `Experiment.run()` 的语义；
2. 以 P2 账本约束训练、估计、两次校准和冻结正常测试的固定顺序；
3. 完成 PCA、DAE 和一步动态预测三类正常数据基线及可重放产物；
4. 通过 P3 相关验证和双轴审查后形成独立提交，再进入 P4。
