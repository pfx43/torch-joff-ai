...
flowchart LR
    ...
    T --> E["evaluation<br/>通用指标、受保护监视、堆叠残差"]
...
| 评估层 | `evaluation` | 预测、标签、重构量、在线记录或潜变量 | 结构化报告、监视 trace 和堆叠残差 | 通用指标、受保护参考与冻结模型后的在线监视状态 |
...
- P4 已提供可学习 Attention--Koopman--T--S 正常模型，P5 已提供受保护在线参考、状态机
  和堆叠残差；P6 算子包、认证 enclosure 与认证状态尚未实现。正常验证诊断或 P5
  ``UNVERIFIED`` anchor 存在不等于真实数据质量 gate 或 anchor 正常性已通过。
