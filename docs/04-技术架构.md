...
> **版本边界**：本文中的 P4–P11、受保护监视、white-space 后滤波、认证算子和物理 explanation graph 都是在老师最新版出现前已经实现的运行时事实。它们继续保留并可复用，但不代表当前论文 V2 主线。尚未实现的测量解耦正常参考、`u/y/z` basis、多尺度分数和兼容集合等能力只在《[论文代码实施计划](./06-论文代码实施计划.md)》中规划；实现与验证发生前不得写入本文的当前架构。
...
## 12. 历史 P0–P11 论文实现的当前落点

上一版“受保护 Koopman–T–S 未知故障检测与结构化隔离”已经按下列分层落地。此表只说明现有代码事实，不规划 V2：
...
- 闭环 CSTR 数据卡把 MathWorks File Exchange #66189 的版本、BSD-3-Clause 许可证 URL/
  文本 hash、本地 Simulink 模型 hash 和完整第三方 notice 固定为**上游模型**来源证据；
  该证据不能自动覆盖仓库内 normal/fault MAT。MAT 虽由入口配置的独立 raw SHA-256 固定，
  但其生成链和再分发许可仍为 ``to_verify``，development/frozen 入口都会先 fail closed。
  paper 入口还把精确数据集卡 SHA-256 纳入 resolved config；读取 raw 前会交叉核对卡片名称、
  raw root、normal/fault 相对路径和许可状态。若状态为 ``verified``，卡片还必须把两个 raw
  hash、MAT 生成记录和许可证据文件连成可离线复验的 hash 链。配置自报 ``verified``、单独
  改卡片状态或同步重签卡片都不能绕过缺失证据。所有路径在跟随现有符号链接后再做
  containment：卡片与证据必须位于仓库或声明的数据根，normal/fault 子文件必须留在数据根；
  绝对路径和 ``..`` 不能扩大访问范围。该机制只固定证据身份与路径边界，不代替人工法律审查。
  即使后续补齐数据许可，``formal_pipeline_complete=false`` 的 runtime 仍不能升格为正式
  evaluator，checkpoint header 与三类认证产物门禁继续独立生效。
