...
| 输入调度确定性包络和有限 episode 动态阈值 | `evaluation/dynamic_threshold.py`（P8 已实现，双轴审查中） | estimate 输入/年龄包络与 detection-calibration 分位严格分开；完整 family、有限秩、正 floor、严格超限和 ``+infinity`` 失败关闭 |
...
- P4 已提供可学习 Attention--Koopman--T--S 正常模型，P5 已提供受保护在线参考、状态机
  ...
  detection calibration；仓库仍没有 P9 集合值隔离或真实
  interval/verified-quadrature provider。正常验证诊断、P5 ``UNVERIFIED`` anchor、
...
- 基础 ``FaultDetectionEvaluator`` 仍只提供静态正常阈值和 FAR/MDR/FDR；论文 P8 动态阈值
  位于独立公开对象中，尚未由通用 ``Experiment`` 自动编排，也不包含 P9 结构化隔离。
