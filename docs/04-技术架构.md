...
- 闭环 CSTR 数据卡把 MathWorks File Exchange #66189 的版本、BSD-3-Clause 许可证 URL/
  文本 hash 和本地 Simulink 模型 hash 固定为公开来源证据；normal/fault MAT 则继续由入口
  配置的独立 raw SHA-256 绑定。许可证核验允许 normal-only development，但不能把其中
  ``formal_pipeline_complete=false`` 的 runtime 升格为正式 evaluator；正式入口仍从
  checkpoint header 和三类认证产物 fail closed。
