...
- P4 已提供可学习 Attention--Koopman--T--S 正常模型，P5 已提供受保护在线参考、状态机
  和堆叠残差，P6 已提供名义算子包、联合认证协议和 fail-closed 状态；仓库仍没有真实
  interval/verified-quadrature provider。正常验证诊断、P5 ``UNVERIFIED`` anchor 或
  P6 ``NOMINAL`` 算子都不等于真实数据质量 gate、anchor 正常性或确定性认证已通过。
