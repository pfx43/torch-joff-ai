...
Joff 是一个面向仿真与过程工业数据的 spec-first PyTorch 实验工具包。它把严格配置、可复用模型构建器、训练集域内数据流水线、实验编排、故障诊断评估和论文级绘图能力收束到一个安静、可复现实验的 Python 包里。
...
tests/                 单元测试与集成测试
configs/               示例实验配置
docs/                  项目文档（入口：docs/01-文档导航.md）
datasets/cards/oa/     公开数据集卡片
datasets/raw/oa/       OA 原始数据
datasets/manifest.public.yaml
```

项目文档（研究路线、技术架构、论文方法设计）从 [docs/01-文档导航.md](docs/01-文档导航.md) 开始阅读。
