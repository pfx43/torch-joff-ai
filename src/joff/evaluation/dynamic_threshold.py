...
from scipy.sparse import csr_matrix, eye, hstack  # type: ignore[import-untyped]

from .postfilter import BranchBank, BranchOperator
from .protected_operators import OperatorBundle, OperatorNorm, OperatorStatus
...
class InputDescriptor:
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        参数：
            无。
        返回：
            只含 region 名称和三个有限浮点列表的字典，可直接交给标准 JSON 编码器。
        异常：
            无；字段已在不可变对象构造时验证。
        副作用：
            无；不修改描述符，也不访问任何数据阶段。
    ...
    def from_dict(cls, value: Mapping[str, object]) -> "InputDescriptor":
        ...
        参数：
            value: 必须恰好包含 ``region``、``u``、``delta_u`` 和 ``delta_xi`` 的映射。
        返回：
            通过类型、非空名称与有限向量检查的 ``InputDescriptor``。
        异常：
            字段缺失、出现额外字段、类型错误或向量含非有限值时抛出
            ``TypeError``/``ValueError``。
        副作用：
            无；只解析调用方提供的内存对象。
        """
...
class EnvelopeEvaluation:
    ...
    参数：
        value: 查询得到的非负包络；仅在 ``supported=True`` 时允许为有限值。
        supported: 当前 descriptor 是否落在 estimate-only 拟合支持域内。
        reason: 不支持时的可审计原因；支持时必须为 ``None``。
    返回：
        构造后的不可变查询结果；调用方据此继续半径传播或失败关闭。
    异常：
        数值、支持状态和原因互相矛盾时抛出 ``ValueError``。
    副作用：
        无；构造过程不修改包络模型或输入 descriptor。
    """
...
class InputDependentEnvelope:
    ...
    参数：
        ...
        maximum_solver_nonzeros: 每个 region 稀疏求解约束的非零元素预算。
    ...
    def __post_init__(self) -> None:
        ...
        if (
            type(self.maximum_solver_nonzeros) is not int
            or self.maximum_solver_nonzeros < 1
        ):
            raise ValueError(
                "Input envelope maximum_solver_nonzeros must be a positive integer."
            )
    ...
    def fit(
        ...
        *,
        ...
        minimum_region_samples: int,
        source_hash: str,
        maximum_solver_nonzeros: int = 1_000_000,
        ...
        参数：
            ...
            maximum_solver_nonzeros: 单个 region 稀疏约束矩阵的非零元素预算；超限时
                在分配求解矩阵前拒绝。
        返回：
        ...
        if type(maximum_solver_nonzeros) is not int or maximum_solver_nonzeros < 1:
            raise ValueError(
                "Input envelope maximum_solver_nonzeros budget must be a positive integer."
            )
        ...
        for region_name in region_names:
            ...
            coefficients = _fit_nonnegative_quantile(
                ...
                maximum_solver_nonzeros=maximum_solver_nonzeros,
            )
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        参数：
            无。
        返回：
            包含 estimate 身份、求解预算、region 系数、支持范围与样本数的字典。
        异常：
            无；对象的不变量已在构造或拟合时验证。
        副作用：
            无；不重新拟合包络，也不读取 detection、attribution 或 fault 数据。
        """
    ...
    def from_dict(cls, value: Mapping[str, object]) -> "InputDependentEnvelope":
        ...
        参数：
            value: 由 ``to_dict`` 产生的完整冻结包络映射；字段集合必须严格一致。
        返回：
            重建且重新通过 estimate 阶段、region、支持范围、样本门槛和求解预算检查的
            ``InputDependentEnvelope``。
        异常：
            映射结构、枚举值、数值类型、SHA-256 身份或任何包络不变量不合法时抛出
            ``TypeError``/``ValueError``。
        副作用：
            无；不会调用线性规划求解器，也不会接触原始 estimate 样本。
        """
        ...
        return cls(
            ...
            maximum_solver_nonzeros=_strict_int(
                value["maximum_solver_nonzeros"],
                name="input envelope maximum_solver_nonzeros",
            ),
...
class ContextAgeEnvelope:
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        参数：
            无。
        返回：
            包含 estimate 身份、逐年龄单调上界与样本门槛证据的字典。
        异常：
            无；对象的不变量已在构造或拟合时验证。
        副作用：
            无；不重新计算经验分位。
    ...
    def from_dict(cls, value: Mapping[str, object]) -> "ContextAgeEnvelope":
        ...
        参数：
            value: 由 ``to_dict`` 产生的完整冻结年龄包络映射。
        返回：
            重新通过 estimate 阶段、连续年龄、单调性、样本门槛和来源身份检查的
            ``ContextAgeEnvelope``。
        异常：
            字段集合、类型、枚举、SHA-256 或曲线不变量不合法时抛出
            ``TypeError``/``ValueError``。
        副作用：
            无；不会读取原始 estimate 记录或重新拟合。
        """
...
class DeterministicRadius:
    ...
    参数：
        ...
        operator_bundle: 生成本半径的完整不可变 P6 算子包，用于严格重放。
        operator_status: P6 算子包的证据级别；不会因本地 SVD 自动升级。
        descriptors/reference_ages: 与路径逐步对齐的在线输入描述符和受保护参考年龄。
    ...
    descriptors: tuple[InputDescriptor, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.operator_bundle, OperatorBundle):
            raise TypeError(
                "Deterministic radius must embed its authoritative P6 operator bundle."
            )
        if not all(
            _is_sha256(value)
            for value in (
                self.score_map_hash,
                self.branch_hash,
                self.operator_bundle_hash,
            )
        ):
            raise ValueError(
                "Deterministic radius score-map/branch/operator identities must be SHA-256."
            )
        if (
            self.operator_bundle_hash != self.operator_bundle.content_hash
            or self.episode_id != self.operator_bundle.path.episode_id
            or self.start_raw_index != self.operator_bundle.path.start_raw_index
            or self.raw_indices != self.operator_bundle.path.raw_indices
            or self.operator_status is not self.operator_bundle.status
        ):
            raise ValueError(
                "Deterministic radius operator bundle, hash, path, and status must agree."
            )
        if (
        ...
        if (
            not isinstance(self.descriptors, tuple)
            or not all(
                isinstance(descriptor, InputDescriptor)
                for descriptor in self.descriptors
            )
            or not isinstance(self.reference_ages, tuple)
            or any(
                type(reference_age) is not int or reference_age < 0
                for reference_age in self.reference_ages
            )
        ):
            raise TypeError(
                "Deterministic radius descriptors and reference ages are invalid."
            )
        if (
        ...
        if (
            len(self.descriptors) != len(self.raw_indices)
            or len(self.reference_ages) != len(self.raw_indices)
            or len(self.source_envelopes) != len(self.raw_indices)
            or len(self.block_norms) != len(self.raw_indices)
        ):
        ...
        if self.supported:
            ...
            with np.errstate(over="ignore", invalid="ignore"):
                expected_radius = float(
                    np.dot(
                        np.asarray(self.source_envelopes, dtype=np.float64),
                        np.asarray(self.block_norms, dtype=np.float64),
                    )
                )
            if (
                not np.isfinite(expected_radius)
                or self.gamma_deterministic != expected_radius
            ):
                raise ValueError(
                    "Supported deterministic radius must equal its derived per-step sum."
                )
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        参数：
            无。
        返回：
            包含 score-map、branch、operator 身份，逐步 descriptor/age、包络、块范数和
            半径状态的字典；正无穷使用明确的 JSON 标记编码。
        异常：
            无；对象构造时已验证派生和与失败关闭状态。
        副作用：
            无；不会重新传播算子或查询输入包络。
        """
        ...
        return {
            ...
            "operator_bundle_hash": self.operator_bundle_hash,
            ...
            "reference_ages": list(self.reference_ages),
    ...
    def from_dict(cls, value: Mapping[str, object]) -> "DeterministicRadius":
        ...
        参数：
            value: 由 ``to_dict`` 产生的完整确定性半径映射。
        返回：
            身份、路径长度、descriptor/age、逐步分账、派生和及 ``+infinity`` 语义均
            重新验证后的 ``DeterministicRadius``。
        异常：
            字段集合、类型、枚举、SHA-256、向量长度或派生半径不一致时抛出
            ``TypeError``/``ValueError``。
        副作用：
            无；只在内存中执行确定性验证，不重新访问 P6/P7 对象。
        """
        ...
        raw_descriptors = _strict_list(
            value["descriptors"],
            name="deterministic radius descriptors",
        )
        raw_reference_ages = _strict_list(
            value["reference_ages"],
            name="deterministic radius reference_ages",
        )
        reason = value["reason"]
        return cls(
            ...
            score_map_hash=_strict_string(
                ...
                name="deterministic radius score_map_hash",
            ),
            branch_hash=_strict_string(
                value["branch_hash"],
                name="deterministic radius branch_hash",
            ),
            operator_bundle_hash=_strict_string(
                value["operator_bundle_hash"],
                name="deterministic radius operator_bundle_hash",
            ),
            operator_bundle=OperatorBundle.from_dict(
                _strict_mapping(
                    value["operator_bundle"],
                    name="deterministic radius operator bundle",
                )
            ),
            ...
            descriptors=tuple(
                InputDescriptor.from_dict(
                    _strict_mapping(
                        item,
                        name="deterministic radius descriptor",
                    )
                )
                for item in raw_descriptors
            ),
            reference_ages=tuple(
                _strict_int(
                    item,
                    name="deterministic radius reference age",
                )
                for item in raw_reference_ages
            ),
    ...
    def compute(
        ...
        *,
        ...
        参数：
            ...
            score_map_hash: calibration 前冻结的 P8 score-map 身份。
        返回：
        ...
        if not _is_sha256(score_map_hash):
            raise ValueError("Deterministic radius score_map_hash must be a SHA-256.")
        branch_hash = _content_hash(branch.to_dict())
        operator_bundle_hash = operator_bundle.content_hash
        ...
        normalized_reference_ages = tuple(reference_ages)
        source_envelopes: list[float] = []
        block_norms, propagation_reasons = _branch_block_norms(
            branch=branch,
            operator_bundle=operator_bundle,
        )
        unsupported_reasons = list(propagation_reasons)
        ...
        if unsupported_reasons:
            return DeterministicRadius(
                ...
                reference_ages=normalized_reference_ages,
...
class ScoreCoordinate:
    ...
    参数：
        time_index: episode 内预声明的非负选择时刻。
        mode: 冻结 monitor state path 上的非空 mode 名。
        branch_name: 冻结 P7 branch family 中的非空支路名。
    返回：
        构造后的不可变且可排序坐标，用于完整 family 校验和稳定 JSON 重放。
    异常：
        时间索引不是非负整数，或 mode/branch 名不是非空字符串时抛出 ``ValueError``。
    副作用：
        无；构造坐标不会运行 monitor、选择支路或读取校准数据。
    """
    ...
    def from_dict(cls, value: Mapping[str, object]) -> "ScoreCoordinate":
        ...
        参数：
            value: 必须恰好包含 ``time_index``、``mode`` 和 ``branch_name`` 的映射。
        返回：
            重新通过非负时间索引与非空名称检查的 ``ScoreCoordinate``。
        异常：
            字段集合、整数类型或名称不合法时抛出 ``TypeError``/``ValueError``。
        副作用：
            无。
        """
...
class DetectionScore:
    ...
    参数：
        radius: 与本分数同一窗口的完整 ``DeterministicRadius`` 证据；其中绑定
            score-map、branch、operator 和逐步派生和。
    ...
    def __post_init__(self) -> None:
        """绑定完整半径证据，重算 normalized excess 并验证失败关闭状态。"""

        if not isinstance(self.radius, DeterministicRadius):
            raise TypeError("Detection score radius must be a DeterministicRadius.")
        ...
        if (
            self.score_map_hash != self.radius.score_map_hash
            or self.episode_id != self.radius.episode_id
            or self.coordinate.branch_name != self.radius.branch_name
            or self.gamma_anchor != self.radius.gamma_anchor
            or self.gamma_deterministic != self.radius.gamma_deterministic
            or self.supported is not self.radius.supported
            or self.reason != self.radius.reason
        ):
            raise ValueError(
                "Detection score components must match the embedded deterministic radius."
            )
        if (
            self.statistic < 0.0
        ...
        if self.supported:
            ...
            if self.reason is not None or self.normalized_excess != expected:
    ...
    def from_components(
        ...
        *,
        ...
        """从完整确定性半径证据与阈值尺度计算可观测超额分数。

        参数：
            radius: ``DeterministicRadiusGenerator`` 产生的完整路径半径证据。
            coordinate: 冻结 finite family 坐标；branch 必须与 ``radius`` 一致。
            statistic/scale: 分数公式中的可观测统计量与 estimate-only 正尺度。
        返回：
            可供 episode maximum 校准的 ``DetectionScore``。
        异常：
            半径类型、branch 身份或字段不满足构造不变量时抛出
            ``TypeError``/``ValueError``。
        副作用：
            无。
        """
        ...
        if not isinstance(coordinate, ScoreCoordinate):
            raise TypeError("coordinate must be a ScoreCoordinate.")
        if coordinate.branch_name != radius.branch_name:
            raise ValueError("Detection score coordinate must match radius branch.")
        ...
        return cls(
            radius=radius,
            score_map_hash=radius.score_map_hash,
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        参数：
            无。
        返回：
            包含 episode 坐标、阈值分账、派生超额分数与 coverage 状态的字典；
            不支持分数中的正无穷使用明确 JSON 标记编码。
        异常：
            无；对象构造时已重算超额分数并验证状态。
        副作用：
            无；不会改变 score map 或 calibration 状态。
        """
    ...
    def from_dict(cls, value: Mapping[str, object]) -> "DetectionScore":
        ...
        参数：
            value: 由 ``to_dict`` 产生的完整 detection-score 映射。
        返回：
            身份、分账、coverage 状态和派生 ``normalized_excess`` 均重新验证后的
            ``DetectionScore``。
        异常：
            字段集合、类型、SHA-256、坐标、有限性或派生分数不一致时抛出
            ``TypeError``/``ValueError``。
        副作用：
            无；不会执行在线检测或写入 episode 校准器。
        """
        ...
        reason = value["reason"]
        return cls(
            radius=DeterministicRadius.from_dict(
                _strict_mapping(value["radius"], name="detection radius")
            ),
            score_map_hash=_strict_string(
                value["score_map_hash"],
...
class CalibrationStatus(str, Enum):
    ...
    参数：
        value: 枚举字符串值；只接受本类声明的三个受控状态。
    返回：
        对应的 ``CalibrationStatus`` 成员，用于区分有限阈值、分辨率不足和证据不完整。
    异常：
        从未知字符串构造时由 ``Enum`` 抛出 ``ValueError``。
    副作用：
        无；枚举构造不读取数据，也不改变校准器状态。
    """
...
class EpisodeMaxCalibrator:
    ...
    参数：
        ...
        score_map: 校准前冻结的完整 ``DynamicThresholdGenerator`` 快照，用于在拟合、
            直接构造与 JSON 重放时重新验证每个分数的实际 branch 分账。
        score_map_hash/reset_state_hash: 上述快照的派生 score/状态身份。
        episode_definition_hash/exchangeability_assumption_hash/source_hash: 有限 episode
            定义、可交换性条件声明和独立 detection-calibration 数据身份。
        reserved_attribution_source_hash/reserved_attribution_episode_ids: 预留 attribution
            calibration 分区的身份元数据，只用于证明两次校准不复用来源或 episode。
    ...
    rank: int
    quantile: float
    score_map: "DynamicThresholdGenerator"
    score_map_hash: str
    reset_state_hash: str
    episode_definition_hash: str
    exchangeability_assumption_hash: str
    source_hash: str
    reserved_attribution_source_hash: str
    reserved_attribution_episode_ids: tuple[str, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.score_map, DynamicThresholdGenerator):
            raise TypeError(
                "Episode maximum calibration must embed a DynamicThresholdGenerator."
            )
        ...
        if not all(
            ...
            for value in (
                ...
                self.source_hash,
                self.reserved_attribution_source_hash,
            )
        ):
            raise ValueError("Calibration identities must be 64-character SHA-256 values.")
        if self.source_hash == self.reserved_attribution_source_hash:
            raise ValueError(
                "Detection calibration source must differ from reserved attribution source."
            )
        if (
            self.score_map_hash != self.score_map.content_hash
            or self.reset_state_hash != self.score_map.reset_state_hash
            or self.expected_coordinates != self.score_map.expected_coordinates
        ):
            raise ValueError(
                "Calibration score-map snapshot, identities, and complete family must agree."
            )
        if (
            not isinstance(self.reserved_attribution_episode_ids, tuple)
            or not self.reserved_attribution_episode_ids
            or any(
                not isinstance(episode_id, str) or not episode_id
                for episode_id in self.reserved_attribution_episode_ids
            )
            or len(set(self.reserved_attribution_episode_ids))
            != len(self.reserved_attribution_episode_ids)
            or self.reserved_attribution_episode_ids
            != tuple(sorted(self.reserved_attribution_episode_ids))
        ):
            raise ValueError(
                "Reserved attribution episode identities must be non-empty, unique, and sorted."
            )
        if (
        ...
        if set(self.episode_ids) & set(self.reserved_attribution_episode_ids):
            raise ValueError(
                "Detection and reserved attribution calibration episode identities overlap."
            )
        ...
        for episode_id, scores, stored_maximum in zip(
            ...
            if any(
                score.radius.operator_bundle.path.stage
                is not MonitorStage.DETECTION_CALIBRATION
                for score in scores
            ):
                raise ValueError(
                    "Every stored calibration score operator path must belong to the "
                    "detection calibration stage."
                )
            for score in scores:
                self.score_map.validate_score(score)
            ...
            if computed_maximum != stored_maximum:
                raise ValueError(
    ...
    def risk_resolution(self) -> float:
        ...
        参数：
            无。
        返回：
            由冻结 calibration episode 数量决定的严格正浮点分辨率。
        异常：
            无；构造器已保证至少保留一个完整 episode maximum。
        副作用：
            无。
        """
    ...
    def fit(
        ...
        *,
        ...
        source_hash: str,
        reserved_attribution_source_hash: str,
        reserved_attribution_episode_ids: Sequence[str],
        ...
        参数：
            ...
            score_map: 校准前冻结的 ``DynamicThresholdGenerator``；完整坐标 family、
                score-map hash 和 reset identity 都从该对象读取，调用方不能缩减。
            stage: 只允许 ``detection_calibration``。
            error_rate: episode family-wise 风险预算 ``alpha``。
            episode_definition_hash/exchangeability_assumption_hash/source_hash: 有限 episode
                定义、可交换性条件声明和 detection-calibration 数据来源。
            reserved_attribution_source_hash/reserved_attribution_episode_ids: attribution
                分区的身份元数据；不得与本次 detection calibration 重用。
        返回：
            READY、INSUFFICIENT_RESOLUTION 或 INCOMPLETE_EVIDENCE 校准对象。
        异常：
            阶段、容器、风险、hash 或基础身份非法时抛出 ``TypeError``/``ValueError``；
            任一分数的权威 P6 路径不属于 detection calibration 时也立即拒绝；
        ...
        if not isinstance(score_map, DynamicThresholdGenerator):
            raise TypeError("Calibration score_map must be a DynamicThresholdGenerator.")
        coordinates = score_map.expected_coordinates
        score_map_hash = score_map.content_hash
        reset_state_hash = score_map.reset_state_hash
        ...
        if not all(
            ...
            for value in (
                ...
                source_hash,
                reserved_attribution_source_hash,
            )
        ):
            raise ValueError("Calibration hashes must be 64-character SHA-256 values.")
        if source_hash == reserved_attribution_source_hash:
            raise ValueError(
                "Detection calibration source must differ from reserved attribution source."
            )
        if isinstance(reserved_attribution_episode_ids, (str, bytes)) or not isinstance(
            reserved_attribution_episode_ids,
            Sequence,
        ):
            raise TypeError(
                "reserved_attribution_episode_ids must be a sequence of strings."
            )
        normalized_attribution_ids = tuple(sorted(reserved_attribution_episode_ids))
        if (
            not normalized_attribution_ids
            or any(
                not isinstance(episode_id, str) or not episode_id
                for episode_id in normalized_attribution_ids
            )
            or len(set(normalized_attribution_ids)) != len(normalized_attribution_ids)
        ):
            raise ValueError(
                "Reserved attribution episode ids must be non-empty and unique."
            )

        expected_set = set(coordinates)
        episode_ids = tuple(sorted(episodes))
        if set(episode_ids) & set(normalized_attribution_ids):
            raise ValueError(
                "Detection and reserved attribution calibration episode ids overlap."
            )
        maxima: list[float] = []
        ...
        for episode_id in episode_ids:
            ...
            if any(
                ...
                raise ValueError(
                    "Every calibration score operator path must belong to the "
                    "detection calibration stage."
                )
            for score in scores:
                score_map.validate_score(score)
        ...
        return cls(
            ...
            source_hash=source_hash,
            reserved_attribution_source_hash=reserved_attribution_source_hash,
            reserved_attribution_episode_ids=normalized_attribution_ids,
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        参数：
            无。
        返回：
            包含完整冻结 score-map 快照、detection/attribution 分区身份、完整坐标族、
            逐 episode 分数、maxima、有限秩和校准状态的字典；不可实现分位以明确的
            正无穷标记编码。
        异常：
            无；所有校准证据已在构造或拟合时交叉验证。
        副作用：
            无；不会重排 episode、重算分位或读取保留归因分数。
        """
        ...
        return {
            ...
            "reserved_attribution_source_hash": self.reserved_attribution_source_hash,
            "reserved_attribution_episode_ids": list(
                self.reserved_attribution_episode_ids
            ),
            "expected_coordinates": [
    ...
    def from_dict(cls, value: Mapping[str, object]) -> "EpisodeMaxCalibrator":
        ...
        参数：
            value: 由 ``to_dict`` 产生的完整有限 episode 校准证据映射。
        返回：
            完整 family、分区独立性、episode maxima、有限秩、分位和状态均重新验证后的
            ``EpisodeMaxCalibrator``。
        异常：
            字段集合、类型、身份哈希、episode 覆盖、检测/归因重叠、秩或分位不一致时
            抛出 ``TypeError``/``ValueError``。
        副作用：
            无；只重放已有证据，不访问原始 calibration 或 fault 数据。
        """
        ...
        raw_attribution_episode_ids = _strict_list(
            value["reserved_attribution_episode_ids"],
            name="calibration reserved_attribution_episode_ids",
        )
        ...
        return cls(
            ...
            score_map=DynamicThresholdGenerator.from_dict(
                _strict_mapping(value["score_map"], name="calibration score_map")
            ),
            score_map_hash=_strict_string(
                value["score_map_hash"],
            ...
            reserved_attribution_source_hash=_strict_string(
                value["reserved_attribution_source_hash"],
                name="calibration reserved_attribution_source_hash",
            ),
            reserved_attribution_episode_ids=tuple(
                _strict_string(
                    item,
                    name="calibration reserved attribution episode id",
                )
                for item in raw_attribution_episode_ids
            ),
...
class ThresholdStatus(str, Enum):
    ...
    参数：
        value: 枚举字符串值；只接受 ``ready`` 或 ``disabled``。
    返回：
        对应的 ``ThresholdStatus`` 成员，供在线判决显式区分可用阈值和失败关闭。
    异常：
        从未知字符串构造时由 ``Enum`` 抛出 ``ValueError``。
    副作用：
        无；枚举构造不运行 score map，也不修改 monitor state。
    """
...
class ThresholdResult:
    ...
    def __post_init__(self) -> None:
        ...
        if self.status is ThresholdStatus.READY:
            ...
            if (
                self.calibration_component != expected_component
                or self.threshold != expected_threshold
                or self.normalized_excess != expected_excess
                or self.alarm is not (self.statistic > expected_threshold)
            ):
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        参数：
            无。
        返回：
            包含全部身份、统计量、三项阈值分账、严格超限位和禁用原因的字典；
            失败关闭的正无穷量使用明确 JSON 标记编码。
        异常：
            无；对象构造时已重算分账与报警判决。
        副作用：
            无；不会更新 monitor state 或触发外部告警。
        """
    ...
    def from_dict(cls, value: Mapping[str, object]) -> "ThresholdResult":
        ...
        参数：
            value: 由 ``to_dict`` 产生的完整动态阈值结果映射。
        返回：
            身份、状态、三项阈值分账、派生超额和严格报警位均重新验证后的
            ``ThresholdResult``。
        异常：
            字段集合、类型、枚举、身份哈希、数值分账或报警位不一致时抛出
            ``TypeError``/``ValueError``。
        副作用：
            无；重放不会产生告警或改变任何冻结对象。
        """
...
class DynamicThresholdGenerator:
    ...
    参数：
        ...
        time_indices/mode_names/reset_state_hash: 有限时间与 mode family，以及确定性
            monitor reset 身份。
    返回：
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not isinstance(self.time_indices, tuple)
            or not self.time_indices
            or any(type(index) is not int or index < 0 for index in self.time_indices)
            or len(set(self.time_indices)) != len(self.time_indices)
            or self.time_indices != tuple(sorted(self.time_indices))
        ):
            raise ValueError(
                "Dynamic threshold time family must be non-empty, unique, and sorted."
            )
        if (
        ...
        *,
        ...
        参数：
            ...
            threshold_floor/normalization_source_hash: 正 floor 与 scale/floor 的 estimate
                来源 hash。
            time_indices/mode_names/reset_state_hash: 有限时间、mode 与 monitor reset。
            stage: 只允许 ``estimate``。
        返回：
        ...
        if isinstance(time_indices, (str, bytes)) or not isinstance(
            time_indices,
            Sequence,
        ):
            raise TypeError("Dynamic threshold time_indices must be an integer sequence.")
        normalized_times = tuple(time_indices)
        if (
            not normalized_times
            or any(type(index) is not int or index < 0 for index in normalized_times)
            or len(set(normalized_times)) != len(normalized_times)
        ):
            raise ValueError(
                "Dynamic threshold time_indices must be unique non-negative integers."
            )
        ...
        return cls(
            ...
            time_indices=tuple(sorted(normalized_times)),
    ...
    def content_hash(self) -> str:
        """返回校准前 score map 的确定性 SHA-256。

        参数：
            无。
        返回：
            candidate、完整 branch bank、两类 estimate 包络、尺度、正 floor、冻结坐标族
            和 reset state 的联合 SHA-256；明确不包含任何 calibration quantile。
        异常：
            无；组成对象已在生成器构造时验证为可标准 JSON 编码。
        副作用：
            无；每次调用只从冻结字段确定性重算哈希。
        """
    ...
    def scale_for(self, branch_name: str) -> float:
        ...
        参数：
            branch_name: 必须属于 calibration 前冻结且启用的 branch family。
        返回：
            与该 branch 同位置保存的有限正归一化尺度。
        异常：
            名称未知或对应 branch 未启用时抛出 ``KeyError``。
        副作用：
            无。
        """
    ...
    def branch_content_hash(self, branch_name: str) -> str:
        """返回冻结 branch 完整部署内容的 SHA-256。

        参数：
            branch_name: 必须属于启用 family 的稳定名称。
        返回：
            branch 的矩阵、kind、anchor 半径和启用状态联合内容哈希。
        异常：
            名称不属于启用 family 时抛出 ``KeyError``。
        副作用：
            无。
        """
        ...
        branch = next(
            (
                item
                for item in self.branch_bank.branches
                if item.enabled and item.name == branch_name
            ),
            None,
        )
        if branch is None:
        ...
        return _content_hash(branch.to_dict())

    @property
    def expected_coordinates(self) -> tuple[ScoreCoordinate, ...]:
        """返回 calibration 前冻结的完整时间、mode 和 branch 笛卡尔积。

        参数：
            无。
        返回：
            按 ``time_indices``、``mode_names``、``branch_names`` 冻结顺序展开的坐标。
        异常：
            无；三个 family 已在构造时验证。
        副作用：
            无。
    ...
    def validate_score(self, score: DetectionScore) -> None:
        """重放并验证一个分数确实来自本冻结 score map。

        参数：
            score: 待进入 detection episode calibration 或在线判决的完整分数证据。
        返回：
            无；验证成功即正常返回。
        异常：
            分数类型、坐标、score-map、descriptor/age 包络、P6/P7 几何、anchor 或
            estimate-only 尺度不匹配时抛出 ``TypeError``/``ValueError``。
        副作用：
            无；不会修改分数、生成器或 monitor state。
        """

        if not isinstance(score, DetectionScore):
            raise TypeError("score must be a DetectionScore.")
        if score.coordinate not in self.expected_coordinates:
            raise ValueError("Detection score coordinate is outside the frozen family.")
        radius = score.radius
        branch = next(
            item
            for item in self.branch_bank.branches
            if item.enabled and item.name == score.coordinate.branch_name
        )
        expected_scale = self.scale_for(score.coordinate.branch_name)
        replayed_radius = DeterministicRadiusGenerator(
            input_envelope=self.input_envelope,
            context_age_envelope=self.context_age_envelope,
        ).compute(
            branch=branch,
            operator_bundle=radius.operator_bundle,
            descriptors=radius.descriptors,
            reference_ages=radius.reference_ages,
            score_map_hash=self.content_hash,
        )
        if (
            score.score_map_hash != self.content_hash
            or radius.to_dict() != replayed_radius.to_dict()
            or score.scale != expected_scale
        ):
            raise ValueError(
                "Detection score does not match the frozen score-map components."
            )
    ...
    def score(
        ...
        *,
        ...
        if not isinstance(radius, DeterministicRadius):
            raise TypeError("radius must be a DeterministicRadius.")
        if type(time_index) is not int or time_index not in self.time_indices:
            raise ValueError("Dynamic threshold time_index is not in the frozen family.")
        if mode not in self.mode_names:
            raise ValueError(f"Dynamic threshold mode {mode!r} is not frozen.")
        if radius.score_map_hash != self.content_hash:
            raise ValueError("Deterministic radius score-map identity does not match.")
        expected_branch_hash = self.branch_content_hash(radius.branch_name)
        branch = next(
            item
            for item in self.branch_bank.branches
            if item.enabled and item.name == radius.branch_name
        )
        if (
            radius.branch_hash != expected_branch_hash
            or radius.gamma_anchor != branch.anchor_radius
        ):
            raise ValueError(
                "Deterministic radius branch geometry/anchor does not match the frozen family."
            )
        scale = self.scale_for(radius.branch_name)
        score = DetectionScore.from_components(
            radius=radius,
            coordinate=ScoreCoordinate(
                time_index=time_index,
                mode=mode,
    ...
    def evaluate(
        ...
        *,
        ...
        elif calibration.expected_coordinates != self.expected_coordinates:
            disabled_reason = (
                "Detection calibration does not cover the complete frozen score family."
            )
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        参数：
            无。
        返回：
            包含 candidate、完整 branch bank、estimate 包络、branch 尺度、正 floor、
            完整时间/mode family 与 reset state 的字典；不包含 detection 分位。
        异常：
            无；组成对象已在生成器构造时验证。
        副作用：
            无；不会执行评分、校准或读取任何在线数据。
        """
    ...
    def from_dict(cls, value: Mapping[str, object]) -> "DynamicThresholdGenerator":
        """从严格 JSON 恢复 calibration 前冻结的 score map。

        参数：
            value: 由 ``to_dict`` 产生的完整生成器映射，且不得包含 ``q_det`` 等校准字段。
        返回：
            candidate、branch 几何、包络、尺度、完整坐标族和 reset state 均重新验证后的
            ``DynamicThresholdGenerator``。
        异常：
            字段缺失或额外字段、类型、枚举、身份哈希、branch family、尺度或冻结坐标
            不一致时抛出 ``TypeError``/``ValueError``。
        副作用：
            无；不会恢复或拟合 detection calibration，也不访问 fault 数据。
        """
        ...
        raw_time_indices = _strict_list(
            value["time_indices"],
            name="dynamic threshold time_indices",
        )
        return cls(
            stage=MonitorStage.parse(
            ...
            time_indices=tuple(
                _strict_int(item, name="dynamic threshold time_index")
                for item in raw_time_indices
            ),
            mode_names=tuple(
...
def _branch_block_norms(
    *,
    branch: BranchOperator,
    operator_bundle: OperatorBundle,
) -> tuple[tuple[float, ...], tuple[str, ...]]:
    """以同一个 P7 branch 重放 P6 ``g_nu`` 的逐步 certified block 上界。

    参数：
        branch: 已冻结且启用的 P7 ``BranchOperator``。
        operator_bundle: 与当前窗口绑定的完整 P6 算子包；``CERTIFIED`` 时读取其共享仿射
            ``g_nu`` image、误差半径和对偶范数。
    返回：
        逐时间 block 的非负有限谱范数，以及任何导致该窗口失败关闭的数值原因。
    异常：
        类型、禁用支路、方阵/分块 shape 或 branch 输入宽度矛盾时抛出
        ``TypeError``/``ValueError``。
    副作用：
        无；该函数同时供首次半径生成和校准/JSON 严格重放使用，避免两套几何漂移。
    """

    if not isinstance(branch, BranchOperator):
        raise TypeError("branch must be a BranchOperator.")
    if not branch.enabled:
        raise ValueError("Disabled post-filter branches cannot receive block propagation.")
    if not isinstance(operator_bundle, OperatorBundle):
        raise TypeError("operator_bundle must be an OperatorBundle.")
    horizon = len(operator_bundle.path.raw_indices)
    ...
    propagated_center = g_nu
    propagated_generators: tuple[np.ndarray, ...] = ()
    enclosure_radius = 0.0
    enclosure_norm: OperatorNorm | None = None
    if operator_bundle.status is OperatorStatus.CERTIFIED:
        if operator_bundle.enclosure is None:
            raise ValueError("Certified operator bundle must retain its joint enclosure.")
        g_nu_image = operator_bundle.enclosure.image("g_nu")
        propagated_center = np.asarray(g_nu_image.center, dtype=np.float64)
        propagated_generators = tuple(
            np.asarray(generator, dtype=np.float64)
            for generator in g_nu_image.generators
        )
        enclosure_radius = operator_bundle.enclosure.error_radius
        enclosure_norm = operator_bundle.enclosure.norm

    block_norms: list[float] = []
    unsupported_reasons: list[str] = []
    for index in range(horizon):
        raw_block = propagated_center[
            :,
            index * block_width : (index + 1) * block_width,
        ]
        # 极端有限数值可能在矩阵乘法或 SVD 中溢出。重放必须得到与首次生成相同的
        # 最大有限审计占位和失败原因，不能把异常误解释成零半径。
        with np.errstate(over="ignore", invalid="ignore"):
            ...
            if propagated_generators:
                generator_norms: list[float] = []
                for generator in propagated_generators:
                    generator_block = generator[
                        :,
                        index * block_width : (index + 1) * block_width,
                    ]
                    transformed_generator = np.asarray(
                        branch.transform_operator(generator_block),
                        dtype=np.float64,
                    )
                    try:
                        generator_norms.append(
                            _scaled_matrix_spectral_norm(transformed_generator)
                        )
                    except ValueError as exc:
                        unsupported_reasons.append(
                            f"step {index} numerical certified propagation: {exc}"
                        )
                        generator_norms.append(float(np.finfo(np.float64).max))
                if enclosure_norm is OperatorNorm.ELEMENTWISE_INF:
                    generator_dual_norm = float(
                        np.sum(np.asarray(generator_norms, dtype=np.float64))
                    )
                else:
                    generator_dual_norm = _stable_vector_norm(generator_norms)
                block_norm = float(
                    np.add(
                        block_norm,
                        np.multiply(enclosure_radius, generator_dual_norm),
                    )
                )
                if not np.isfinite(block_norm):
                    unsupported_reasons.append(
                        f"step {index} numerical certified block bound overflow"
                    )
                    block_norm = float(np.finfo(np.float64).max)
        block_norms.append(block_norm)

    return tuple(block_norms), tuple(unsupported_reasons)
...
def _fit_nonnegative_quantile(
    ...
    *,
    ...
    """用受预算约束的稀疏 pinball-loss 线性规划求非负系数。

    参数：
        design: ``(n,d)`` 有限设计矩阵；调用方已加入常数项和非负 descriptor 特征。
        response: 长度 ``n`` 的非负 estimate-only 响应。
        quantile: 位于 ``(0,1)`` 的目标分位；决定正、负 pinball slack 的权重。
        maximum_solver_nonzeros: 稀疏等式约束允许的最大非零元数，必须在分配矩阵前检查。
    返回：
        长度 ``d`` 的有限非负系数；浮点舍入产生的微小负值会截断为零。
    异常：
        稀疏约束超预算、HiGHS 求解失败或系数非有限时抛出 ``ValueError``。
    副作用：
        调用 SciPy HiGHS 求解器并分配预算内的稀疏工作区；不修改输入数组或全局状态。
    """

    sample_count, feature_count = design.shape
    constraint_nonzeros = int(np.count_nonzero(design)) + 2 * sample_count
    if constraint_nonzeros > maximum_solver_nonzeros:
        raise ValueError(
            "Input-envelope sparse solver budget exceeded before constraint allocation: "
            f"required_nonzeros={constraint_nonzeros}, "
            f"maximum_solver_nonzeros={maximum_solver_nonzeros}."
        )
    ...
    identity = eye(sample_count, dtype=np.float64, format="csr")
    equality = hstack(
        (
            csr_matrix(design),
            identity,
...
def _stable_vector_norm(values: Sequence[float]) -> float:
    ...
    参数：
        values: 可转换为 ``float64`` 的一维有限实数序列。
    返回：
        非负 2-范数；全零或空序列返回 ``0.0``。
    异常：
        缩放后的范数仍无法表示为有限 ``float64`` 时抛出 ``ValueError``。
    副作用：
        无；只创建局部 NumPy 数组，不修改输入序列。
    """
...
def _scaled_matrix_spectral_norm(matrix: np.ndarray) -> float:
    ...
    参数：
        matrix: 待传播 block 经 P7 ``L_b`` 变换后的二维有限矩阵。
    返回：
        最大奇异值；空矩阵或全零矩阵返回 ``0.0``。
    异常：
        输入不是有限二维矩阵、SVD 不收敛或结果无法表示时抛出 ``ValueError``，使上层
        半径生成与重放统一失败关闭。
    副作用：
        调用 NumPy SVD 并分配局部缩放矩阵；不修改原矩阵或 branch 状态。
    """
...
def _input_envelope_payload(envelope: InputDependentEnvelope) -> dict[str, object]:
    ...
    return {
        ...
        "maximum_solver_nonzeros": envelope.maximum_solver_nonzeros,
