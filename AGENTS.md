<!--
文件用途：本文件是 paper_project 仓库中所有自动化代理的唯一根级协作入口。
主要职责：规定仓库权威信息源、开发与验证命令、运行时架构不变量、论文与数据边界、
文献检索流程、交付要求、事项追踪方式，以及修改文件时必须遵守的中文注释规范。
维护要求：仓库级协作规则发生变化时只更新本文件；不得另建内容重复的工具专用入口。
-->
...
1. `docs/文档导航.md`：项目文档的唯一入口。
2. `MANUSCRIPT_CONTEXT.md`：英文论文的项目级写作上下文；处理 `paper/` 前必须整读。
3. `docs/当前工作记录.md`：当前阶段、阻塞、验证结果和近期决策。
4. `docs/技术架构.md`：运行时分层、调用链、扩展接口和架构不变量。
5. `docs/目录结构与文件数.md`：仓库目录、核心文件地图、文件统计口径和源码阅读顺序。
...
分层职责、扩展方式和当前限制以 `docs/技术架构.md` 为准，不在本文件重复完整代码地图。

## 4. 数据与论文硬约束

论文遵守“仅正常数据”纪律：训练、结构选择、超参数选择、窗口选择、阈值校准、anchor gate 和状态策略冻结都不得使用故障数据。故障数据只能进入协议冻结后的最终测试。

当前理论要求五阶段数据协议：前四阶段只含正常数据，第五阶段才使用冻结故障测试；各段之间保留隔离带：

`正常训练 -> 正常估计 -> 检测校准 -> 归因校准 -> 冻结故障测试`
...
新增或重命名文档必须登记到 `docs/文档导航.md`。只有改变研究判断、阶段、阻塞或关键证据的里程碑才写入 `docs/当前工作记录.md`；普通调试运行留在运行产物中。
...
以后新建或修改任何**由项目维护、且文件格式允许注释的文本文件**时，必须在文件顶部补充或更新详细的中文说明。已有文件缺少顶部说明时，本次修改必须一并补齐，不能以“不是本次功能的一部分”为由跳过。
...
| PowerShell、Shell、配置文件 | 文件顶部连续 `#` 中文注释；shebang 必须保持第一行 |
| LaTeX | 文件顶部连续 `%` 中文注释，放在 `\documentclass` 之前 |
| Markdown | 文件标题之前的 HTML 注释 `<!-- ... -->` |
| JavaScript、TypeScript、C/C++、Java 等 | 文件顶部块注释，放在语言要求的固定声明之后 |

不能安全容纳注释的文件不得强行插入内容：

- JSON、锁文件、数据文件、模型权重、图片、PDF 和其他二进制文件保持原格式；
- 自动生成文件和第三方 vendored 文件不得手工加注释；
- 这些文件的用途应记录在同目录 README、manifest、dataset card 或最近的人工维护入口文档中。
