...
- 所有新建 Git 提交的标题和正文必须使用中文，不能使用仅含英文的 Conventional Commits
  前缀或英文句子；
- 文件路径、命令、类名、函数名、配置键和必要的标准技术术语可以保留英文；
- 提交标题应直接说明本次修改的对象与结果，例如“实验：绑定数据集卡证据链”，不能写成
  “update files”或其他无法判断范围的笼统表述；
- 自动化代理在提交前必须检查最终提交信息；合并远程分支时，合并提交信息也必须使用中文。
