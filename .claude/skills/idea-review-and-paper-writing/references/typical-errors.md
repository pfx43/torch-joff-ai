...
| Repeating short `The method ...` declarative sentences throughout a paragraph | Produces rigid, generated-sounding prose and hides whether ideas are causal, contrastive, conditional, or merely adjacent. | Select voice and syntax from the actual relation; use clauses, nonfinite constructions, coordination, prepositional openers, or adverbials only when they make that relation clearer. |
| Forcing passive voice, long clauses, or decorative adjectives merely to create sentence variety | Replaces one mechanical style with another and can obscure agency, conditions, or evidence. | Vary structure selectively, split overloaded sentences, and retain only precise evidence-bearing modifiers. |
| Presenting a module result before explaining the problem, condition, or purpose that requires it | Makes the proposed structure appear arbitrary and reverses the scientific dependency. | State the cause, limitation, or objective first, then the responsive construction, mechanism, consequence, and evidence; reserve conclusion-first order for a deliberately scoped clarification. |
| Describing detailed modules before stating the overall model mapping and information flow | Forces the reader to infer the system-level role of each component from local details. | Introduce the macro-level question, inputs, outputs, and overall mapping before decomposing components, equations, training, and tests. |
| Retaining a paragraph with no single theme or reader-facing output | Adds length without defining, motivating, deriving, explaining, validating, delimiting, or bridging anything needed by the main line. | Assign one purpose in the paragraph map; delete the paragraph or merge its indispensable content when no distinct purpose remains. |
