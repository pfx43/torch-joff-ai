...
For IEEE work, start from `bare_jrnl.tex` with `IEEEtran.cls` unless the exact IEEE publication supplies a more specific template. The registered journal class defaults to a two-column article body. Respect the source warning against changing margins, column widths, line spacing, or fonts. Retain the IEEEtran notices and identify any permitted modification when redistribution requires it.
